...
def saveDataIntoText(name):
    ...
    text_file.write(time.strftime("%c")+","+data['condition']['date'][-23:]+","+data['astronomy']['sunrise']+","+data['astronomy']['sunset']+","+data["condition"]["temp"]+","+data["condition"]["text"]+","+data["atmosphere"]["humidity"]+","+data["atmosphere"]["pressure"]+","+data["atmosphere"]["rising"]+","+data["atmosphere"]["visibility"]+","+data["wind"]["chill"]+","+data["wind"]["direction"]+","+data["wind"]["speed"]+"\n")
    text_file.close()
